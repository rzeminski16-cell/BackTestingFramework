...
Entry: RSI oversold and price bounces off support
...
class PartialExitStrategy(BaseStrategy):
    ...
    Entry when RSI is oversold and recovering
    Takes partial profits at 10% and 20% gains
    Final exit when RSI becomes overbought or stop loss hit
    ...
    def required_columns(self) -> List[str]:
        ...
        return ['date', 'close', f'rsi_{self.rsi_period}']
    ...
    def generate_signal(self, context: StrategyContext) -> Signal:
        ...
        if not context.has_position:
            ...
            if context.previous_bar is not None:
                previous_rsi = context.get_indicator_value(f'rsi_{self.rsi_period}', offset=-1)
